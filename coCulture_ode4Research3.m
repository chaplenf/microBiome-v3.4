function dy = coCulture_ode4Research3(t, y, ~)
...
uptakeRates14;  % updating FBA organism uptake constraints
...
try
    switch community.species{1,i}.description
        case 'Nitrosomonas europaea'
               community.species{1,i} = changeRxnBounds(community.species{1,i},'EX_cpd00418x_c',1.5,'u');
               community.species{1,i} = changeRxnBounds(community.species{1,i},'EX_cpd00418_e',1.5,'u'); 
               community.species{1,i} = changeRxnBounds(community.species{1,i},'EX_cpd00418_e',0,'l');  
               community.species{1,i} = changeObjective(community.species{1,i},'EX_cpd11416_e',1);     
     [vNeu]=optimizeCbModel(community.species{1,i},'max',1e-5);
            if (vNeu.stat ~= 1)    
               ...
               community.species{1,i} = changeRxnBounds(community.species{1,i},'EX_cpd11416_e',0.0,'u');             
                disp('deathphase');
            else
